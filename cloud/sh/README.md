...
- **Random WebSocket disconnects (e.g. Guacamole sessions dropping unpredictably) can be caused by HTTP/3 (QUIC) on the browser-to-Cloudflare-edge leg**, independent of anything in `cloudflared`'s own config — `protocol: http2` in `config.yml` only controls the VPS-to-Cloudflare tunnel connection, not the separate connection your browser negotiates with Cloudflare's edge. If a network's handling of QUIC/UDP is unreliable (the same class of problem covered above for Tailscale's STUN), it can cause WebSocket upgrades to fail or drop with no error logged anywhere in `cloudflared` or the origin server. Fix: **Cloudflare dashboard -> your domain -> Speed -> Settings -> turn off "HTTP/3 (with QUIC)"** for the zone.
